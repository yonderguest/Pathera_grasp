...
本轮在已推送的 `803271b` 大修前检查点上完成以下修改。代码已经通过离线回归，但尚未执行机械臂运动，因此仍不能描述为真机验收通过：
...
- 到达 `15..40 mm` 安全预抓取退距并完成笛卡尔对正后，新增三帧近距位置复检。近距画面只修正同一积木的 Base XYZ，沿用远距阶段已经稳定的抓取姿态；目标深度小于 `75 mm`、框触边、框面积超过画面 `30%`、深度离散超过 `15 mm`、三帧位置离散超过 `6 mm` 或总修正超过 `12 mm` 时跳过近距结果，继续使用上一阶段目标。这样既利用近距深度，又不让放大/裁切的 OBB 改坏腕部角度。
...
9. 对正后在安全退距处执行三帧近距位置复检；仅在目标完整可见、深度有效且小修正一致时更新 Base XYZ，抓取姿态保持远距结果。随后按 `2 mm` 采样最终笛卡尔路径；整段必须 100% 完成，逐点通过关节限位、跳变、FK、工作空间、轴向单调、命令线段跑偏和抓取轴横向收敛检查。闭爪前还会读取实机 FK，实际 TCP 对软件目标超过 `12 mm` 时放弃本轮并回识别位。
...
GRASP_OFFSET = [0.010, 0.040, 0.017] m
...
旧补偿 `[0.150, 0.040, -0.070] m` 中混入了 HOME 姿态下缺失的 joint6→TCP 投影 `[0.1402, 0.0000, -0.0869] m`。现在坐标链已经显式包含该刚体变换，因此迁移后的剩余补偿约为两者之差 `[0.0098, 0.0400, 0.0169] m`（配置取毫米级圆整值）。用同一份终端样本离线复算，修正前后的 HOME 观察位 Tool target 基本保持不变，但腕部旋转后的近距复检不再产生约 9–10 cm 的假目标漂移。
...
本次精度修改的正式目录离线结果：147 个 Python 文件内存编译成功，当前单体主线相关的 60 项回归测试全部通过。测试覆盖 NPU NMS、稳健近表面深度、有符号预抓取、预抓取残差对正、近距位置复检保持远距姿态、被动末端停稳、J1 网页转动的空闲态/姿态门控、D405 主动释放、偏轴/越界拒绝、笛卡尔路径单调性、横向收敛允许和真实离线跑偏 fail-closed。已暂停使用的 ROS2 传输测试需要另行加载 Humble `rclpy`，不计入本轮单体主线结果；离线通过仍不代表真机轨迹、碰撞或最终抓取精度已经验收。
